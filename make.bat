...
SETLOCAL
SET PROJECT_DIR=%cd%
SET PROJECT_NAME=ba-testing
SET SUPPORT_LIBRARY = ba_testing
...
:env
    ...
    CALL conda create -p %CONDA_DIR% --clone "%PROGRAMFILES%\ArcGIS\Pro\bin\Python\envs\arcgispro-py3"
...
:build_upload
    ...
    :: Build conda package
    CALL conda build ./conda-recipe --output-folder ./conda-recipe/conda-build
    CALL anaconda upload ./conda-recipe/conda-build/win-64/ba-testing*.tar.bz2

    GOTO end

:: Run all tests in module
:test
	CALL conda run -p %CONDA_DIR% pytest "%~dp0env"
	GOTO end
