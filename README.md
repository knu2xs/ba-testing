...
Scripts and tools to streamline Business Analyst Quality Assurance testing.
...
3 - Explore - If you are more into Python, a good place to start is `jupyter lab` from the root of the project, and look in the `./notebooks` directory. If GIS is more your schtick, open the project `./arcgis/ba-testing.aprx`.
...
- `make env` - builds the Conda environment with all the name and dependencies from `environment_dev.yml` and installs the local project package `ba_testing` using the command `python -m pip install -e ./src/src/ba_testing` so you can easily test against the package as you are developing it.
